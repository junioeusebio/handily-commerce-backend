...
- Default policy allows origins `https://junioeusebio.github.io` (GitHub Pages), `https://handily-commerce-backend.onrender.com` (Render demo / Swagger), and localhost:4200 for Angular dev
- Methods: `GET`, `OPTIONS`
...
- OpenAPI document `Info.Version` / `Info.Title` use the same keys via `AddOpenApi` + document transformer, so Swagger UI (`/swagger`, document at `/openapi/v1.json`) stays aligned.
